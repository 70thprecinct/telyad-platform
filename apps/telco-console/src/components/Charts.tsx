...
// Local to the telco console (no chart library bundled). Presentational only.
...
export function DoughnutChart({
  ...
  const total = data.reduce((s, d) => s + d.value, 0) || 1, r = size / 2, inner = r * 0.62;
  let angle = -Math.PI / 2;
  const arcs = data.map((d) => {
    const frac = d.value / total, a0 = angle, a1 = angle + frac * Math.PI * 2;
    angle = a1;
    const large = a1 - a0 > Math.PI ? 1 : 0;
    const p = (rad: number, ang: number) => [r + rad * Math.cos(ang), r + rad * Math.sin(ang)];
    const [x0, y0] = p(r, a0), [x1, y1] = p(r, a1), [ix1, iy1] = p(inner, a1), [ix0, iy0] = p(inner, a0);
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {data.map((d, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12 }}>
            ...
            <span className="tly-mono" style={{ marginLeft: 'auto', fontWeight: 600 }}>{((d.value / total) * 100).toFixed(1)}%</span>
          </div>
        ))}
      </div>
    </div>
